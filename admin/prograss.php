...
//Pending Users
if($do =='manage'){
echo "<h3 class='table-title'><span>Prograss Bars</span><i class='fa fa-building' aria-hidden='true'></i></h3>";
  //Select Categories Data From database
  $select = sql("SELECT * FROM prograss ORDER By id DESC ","fetchAll");
...
elseif($do == 'insert'){
  ...
  echo '<h3>Insert New</h3>';
    sql("INSERT INTO prograss (title,count)
    VALUES ('$title','$count')","");
...
elseif($do == 'edit'){
  ...
  $select = sql("SELECT * FROM prograss WHERE id = '$id'","fetch");
  ?>
  <h3 class="table-title"><span>Edit Prograss Bars</span><i class="fa fa-users" aria-hidden="true"></i></h3>
  <div class="child-page">
...
elseif($do == 'update'){
  ...
  $id        =$_POST['id'];
  //Update In Database
  $update = sql("UPDATE prograss SET title = '$title',count = '$count'
                  WHERE id = '$id'","");
...
elseif($do == 'delete'){
  ...
  echo"<h1>Delete Prograss Bar</h1>
  <div class='container'>";
  $id = isset($_GET['id']) && is_numeric($_GET['id'])?intval($_GET['id']) : 0;
//Chick If Item Exist In Database
  $check = checkItems("id","prograss",$id);
//Select User Data From database
if ($check == 1){
  $delete = sql("DELETE FROM prograss WHERE id = '$id'","");
