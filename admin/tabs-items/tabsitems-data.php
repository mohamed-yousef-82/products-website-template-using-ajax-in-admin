...
/*---Select Data From Table And View It---*/
 $select = sql("SELECT $table.*,tabs.title AS tab
              FROM $table INNER JOIN tabs ON tabs.id = $table.tab_id
          ...
          <td class="popup" data-popup="popup-<?php echo $selectview['id'] ?>">
            <div class="popup-inner">
              ...
              <select name="tab">
                  ...
                  foreach ($tabs as $tab) {
                    ?>
                  <option value='<?php echo $tab['id']; ?>' <?php if($selectview['tab_id']==$tab['id']){echo 'selected';}  ?>> <?php echo $tab['title'] ?></option>
                  <?php
                }
                  ?>
                </select>
